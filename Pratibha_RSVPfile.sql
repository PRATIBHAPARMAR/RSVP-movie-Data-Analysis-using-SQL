...
SELECT 
     COUNT(*) AS language_null
     from movie
     where languages is null;

SELECT 
     COUNT(*) AS worldwidegrossincome_null
     from movie
...
-- Example To get the month from date_published column
SELECT
...
WITH actress_ratings as (
   ...
   FROM names AS N LEFT JOIN role_mapping AS RM
		ON N.id = RM.name_id
...
SELECT 
    genre,
    ROUND(AVG(duration),2) AS avg_duration ,
    ROUND(AVG(duration),2) as moving_avg_duration
FROM
    genre AS g
        LEFT JOIN
    movie AS m 
...
-- Type your code below:
 ...
 SELECT M.production_company, 
		...
		ROW_NUMBER() OVER(ORDER BY COUNT(M.id) DESC) prod_comp_rank
